...
//     const serviceUrl = "https://api.au-syd.assistant.watson.cloud.ibm.com/instances/1eaa04b9-52bd-4144-b191-26e1dd7f2367"    <!-- Remove or comment out this block in dashboard.html -->
...
//       </div>
//     </div>    <!-- Remove or comment out this block in dashboard.html -->
//     <div class="chat-container">
//       <div class="chat-header">
//         <span class="chat-header-icon">💬</span>
//         ChatBox
...
//       </div>
//     </div>
//     const assistantId = "70d6d0c4-97d6-4bdc-9e86-f00e6808b552";
//     const apiKey = "WKjfdt341bEV0IdTMMvEdmo2JRnT1XXEHzn96Ch2SRqt";
//     const version = "2021-11-27";

//     // 1. Create a session
//     const sessionRes = await fetch(
//       `${serviceUrl}/v2/assistants/${assistantId}/sessions?version=${version}`,
//       {
//         method: "POST",
//         headers: {
...
//       }
    ...
    if (!sessionId) {
      addMessage("Could not create session with Watson Assistant.", false);
      return;
    }

    const messageRes = await fetch(
      `${serviceUrl}/v2/assistants/${assistantId}/sessions/${sessionId}/message?version=${version}`,
      {
        method: "POST",
        headers: {
          "Authorization": "Basic " + btoa("apikey:" + apiKey),
        ...
        body: JSON.stringify({
          ...
          // environment_id: "25ed2a96-f71c-4482-a390-298793a394aa", // Draft
          // environment_id: "f4849f9d-0d5f-400c-ba13-15eb73ce574c", // Live
        }),
      }
    );
    ...
    // Extract the chatbot's reply from IBM Watson response
    let botReply = "Sorry, I couldn't process that request.";
    if (
      data &&
      data.output &&
      data.output.generic &&
      data.output.generic.length > 0
    ) {
      const textResponse = data.output.generic.find(
        (item) => item.response_type === "text"
      );
